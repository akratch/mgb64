...
if [ "$HAVE_GIT" -eq 1 ]; then
  forbidden=$(git ls-files \
    ...
    '*.raw' '*.wav' '*.mp3' '*.ogg' '*.flac' \
    '*.dmg' '*.zip' '*.7z' '*.tar' '*.tgz' '*.gz' 2>/dev/null || true)
else
  forbidden=$(public_file_list \
    | grep -E '(^|/)baserom|\.((z64|n64|v64|bin|rz|eeprom|ctl|tbl|sbk|seq|aifc|aiff|seg|o|a|so|dll|dylib|exe|bmp|png|jpe?g|gif|webp|ico|icns|ppm|raw|wav|mp3|ogg|flac|dmg|zip|7z|tar|tgz|gz))$|\.cdata($|\.)' \
    || true)
fi
if [ -n "$forbidden" ]; then
