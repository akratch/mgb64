...
# WEB-032 (build half): stamp the STAGED shell with a short build hash so it can
# load ge007_web.js?v=HASH / fetch ge007_web.wasm?v=HASH and never pair cached
# glue with fresh wasm across a Pages redeploy. The source shell keeps the
# __MGB64_BUILD__ placeholder — rewrite the staged copy only; if the placeholder
# isn't present yet the sed is a harmless no-op. Portable sed via temp file (not
# -i: the CI runner is Linux, dev machines are macOS/BSD). BUILD_HASH is a git
# short hash or a unix timestamp — both sed-safe (no slashes/metacharacters).
BUILD_HASH="$(git -C "$ROOT" rev-parse --short=8 HEAD 2>/dev/null || date +%s)"
sed "s/__MGB64_BUILD__/$BUILD_HASH/g" "$ROOT/dist/web/mgb64-shell.js" > "$ROOT/dist/web/mgb64-shell.js.tmp" \
    && mv "$ROOT/dist/web/mgb64-shell.js.tmp" "$ROOT/dist/web/mgb64-shell.js"
