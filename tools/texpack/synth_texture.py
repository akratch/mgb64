...
def _bandpass(h, w, cells, rng, bw=0.6):
    ...
    r = np.sqrt((fx * (cells / w)) ** 2 + (fy * (cells / h)) ** 2)
...
def main():
    ...
    g = GEN[args.kind](h, w, rng)
    if target_vals is not None and not args.no_hist and args.mean is None and args.sd is None:
