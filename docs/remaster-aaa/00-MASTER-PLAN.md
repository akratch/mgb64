...
1. **W8.E6.T2 (merge train) is first** — five in-flight branches must consolidate onto `main`
...
3. **W2.E1→E3 (manifest + Router)** gates scaled asset production and the material sidecars
   (W2.E6) that make W1.E5 worth shipping.
4. **W3.E2 SSAO v2 before W1.M4** — so AO isn't re-tuned twice under the new lighting.
...
open docs/REMASTER_ROADMAP.md
ls docs/remaster-aaa/          # pick your workstream doc (01-08) and read it
...
1. **Global rails**: this doc §6 verbatim + `docs/REMASTER_ROADMAP.md` §1.
...
4. **The owning doc's §8 validation ladder** + the gotchas list from
   `docs/superpowers/handoffs/2026-07-02-metal-remaster-aaa-validation-handoff.md` §6
...
- **Remains human** (the orchestrator queues these for the user, never self-approves):
  ◆-marked taste checkpoints in the workstream docs, each doc's §9 open questions,
  every default-flip decision, and any change to a threshold/budget/gate.
