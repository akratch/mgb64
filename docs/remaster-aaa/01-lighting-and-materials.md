...
Branch of record: `feat/metal-backend`. All anchors below were re-verified against that
tree on 2026-07-02 — where they differ from the roadmap's older anchors, **this doc's
numbers win** (the roadmap says its anchors drift; they have).
...
|---|---|---|
| 1 | **Smooth env normals** (T1.3, CPU relight, no VBO change) | Dam cliffs stop reading as "tiger-striped facets"; ground banding gone |
| 2 | **World-space VBO plumbing** (T2.1) | Invisible alone — the gate that unlocks 3–5 |
...
Total honest estimate: **~90 junior-days (≈ 18–20 junior-weeks)** across 7 epics.
...
- `bgLevelRender` explicitly clears lighting for all room geometry:
  `gSPClearGeometryMode(arg0++, G_TEXTURE_GEN | G_LIGHTING)` — `src/game/bg.c:6991`
  (comment at `bg.c:6987`: "GE rooms use baked vertex colors, NOT hardware lighting").
  A second clear site exists at `src/game/lvl.c:1414`.
...
- The global sun **does** exist: `Lights1 GlobalLight` — `src/game/bg.c:2740` — ambient
  ...
  `bg.c:7000`) but has no effect on rooms because G_LIGHTING is cleared.

### 2.2 The render pipeline the design must fit (docs/RENDERING_ARCHITECTURE.md §2)

- Software T&L: `gfx_sp_vertex` (`gfx_pc.c:16059`) transforms object-space `ob[3]`
  straight to **clip space** by `rsp.MP_matrix` (`gfx_pc.c:16084-16087`). The lit path
  (G_LIGHTING, used by characters/weapons, e.g. `bondview.c:18704`, `title.c:941`)
  consumes `Vtx_tn.n[]` on the CPU into a vertex **color** at `gfx_pc.c:16245-16310`.
- `struct LoadedVertex` (`gfx_pc.c:2434-2461`) carries clip pos, uv, color, fog,
  `int16 ob[3]` + `room_id` (populated only when tracing — `gfx_pc.c:16203-16215`), and
  debug fields. **No world position, no normal.**
- Matrix state: modelview stack `rsp.modelview_matrix_stack[11][4][4]` (`gfx_pc.c:3380`),
  `P_matrix` (`:3391`), loaded in `gfx_sp_matrix` (`gfx_pc.c:15389`); MP recomputed at
  `gfx_pc.c:15348-15354`. Projection coefficients for SSAO are already captured per frame
  at `gfx_pc.c:15445-15451` into `g_pc_ssao_proj_{a,b,x,y}` (`:15384-15387`) — the
  pattern to copy for any new per-frame matrix capture.
- Triangles are packed into one interleaved CPU array `buf_vbo`
  (`gfx_pc.c:4478`, packing loop `gfx_pc.c:18390-18552`): pos(4) → [diag(6)] →
  per-texture uv(2)+clamp/mask(0-4) → [fog(4)] → combiner inputs (3 or 4 each).
  `gfx_flush()` (`gfx_pc.c:14053`) hands it to `gfx_rapi->draw_triangles`
...
- Shader identity = `(cc_id, cc_options)`; options assembled per draw at
  `gfx_pc.c:17282-17405`, combiner cached via `gfx_lookup_or_create_color_combiner`
  (`gfx_pc.c:14315`). **Free `SHADER_OPT` bits: 5, 6, 7, 29, 30, 31**
  (`gfx_cc.h:50-75`) — this workstream claims four of them (§4.1).
- GLSL generator: `gfx_opengl_create_and_load_new_shader` (`gfx_opengl.c:819-1449`).
  VS is a near-passthrough (`gl_Position = aVtxPos`, `gfx_opengl.c:926`; z×0.3 spread at
  `:933` when no depth clamp). Attribute walk order `gfx_opengl.c:846-895` == VBO pack
  order; attrib binding `:1349-1397`; samplers `uTex0`→unit 0, `uTex1`→unit 1
  (`:1434-1442`), diag framebuffer→unit 2 (`:1443-1448`). **Units 3+ are free.**
- MSL generator: `mtl_generate_msl` (`gfx_metal.mm:262`), a block-for-block mirror.
  Attribute walk via `add_attr` (`gfx_metal.mm:382-434`) fills `MetalShader.attrs[]`
  consumed by `mtl_build_vertex_descriptor` (`:880-897`). Fragment binds
  `uTex0 [[texture(0)]]`, `uTex1 [[texture(1)]]`, diag snapshot `[[texture(2)]]`
  (`gfx_metal.mm:451-458`); uniforms are one 48-byte `Uniforms` at `[[buffer(1)]]`
  (`:289-297`) with a C mirror — any new uniform must extend **both** structs in lockstep.
  Scene targets: BGRA8 color + **sampleable Depth32Float** (`mtl_ensure_targets`,
  `gfx_metal.mm:797-865`); scene encoder open/close `mtl_open_scene_encoder`
  (`:1007-1019`); frame lifecycle `mtl_start_frame` (`:1074-1122`) / `mtl_end_frame`
  (`:1460`); PSO cache keyed on blend|samples|write_alpha (`mtl_pso_for`, `:901-960`).
- GL scene target: `g_scene_fbo` + sampleable `g_scene_depth_tex`
  (`gfx_opengl.c:648-666`, created in `gfx_opengl_ensure_scene_target`, `:2280`). SSAO
  v1 lives in the output filter (GLSL `gfx_opengl.c:2984-3018`; MSL port
  `gfx_metal.mm:1271-1296`), controlled by `Video.Ssao` et al.
  (`platform_sdl.c:202-212`, registered `:1601-1615`).
...
- **Shoot-out-the-lights**: real hit-detection/darkening exists behind
  ...
  docs/SHOOT_OUT_LIGHTS_PLAN.md) — it darkens fixture *surfaces*; there is no light
...
- **HD texture loader**: `texture_pack_try_load` (`texture_pack.c:64-90`) probes
  `<pack>/textures/tok%04d.png` only; hook at `gfx_pc.c:21061`. No sidecar concept yet.
...
- **Space convention**: all new lighting math is **world space**. `gfx_sp_vertex`
  produces view-adjacent clip coords; world position is recovered per vertex as
  `world = (ob · MV) · V⁻¹` where `V⁻¹` is the inverse camera-view matrix captured once
  per frame (§4.2). Rooms could use the cheaper `filepos + ob` identity visible at
  `gfx_pc.c:16133-16135`, but the general path covers props/chars uniformly; we use it
  everywhere for one code path.
...
- Per-vertex averaging at draw time is impossible with full fidelity: `gfx_sp_vertex`
  sees ≤32-vertex loads (`gfx_pc.c:16059`) and triangles arrive after the load — no
  ...
  `bg.c:11364` — the roadmap's `bg.c:9750` anchor has drifted) and the
  shoot-out-lights walker (`lightfixture.c:137-159` G_VTX backtrack + `:176-179`
...
1. Walk the room's expanded DL exactly as `lightfixture.c` does (backtrack-free forward
   ...
   `lightfixture.c:176-179` documents both strides).
2. For each triangle, compute the face normal from the three **object-space** `ob[]`
   (room-local space is a uniform scale+translate of world — `gfx_pc.c:16133-16135` —
   so object-space normals equal world-space normals for rooms; no matrix needed).
...
`gfx_pc.c:16306-16310`, gated so flag-off costs one predictable branch):**
...
```c
    ...
    if (g_pcEnvSmoothNormals && source_room >= 0 &&
        ...
        if (n) {
             ...
             * its baked brightness: amb=150/255, dif=105/255 (GlobalLight, bg.c:2740,
...
packing at `gfx_pc.c:18480-18492`).
...
**LoadedVertex extension** (`gfx_pc.c:2434`): append
...
**Packing** (`gfx_pc.c:18390` loop): insert immediately after the pos/diag block
(`:18409`), before texcoords:
...
**GL generator** (`gfx_opengl.c`, VS attr block after the aDiagTri block `:853`):
...
`:1349-1397` walk (order == pack order, enforced by the existing convention comment at
`gfx_metal.mm:133-136`).

**Metal generator** (`gfx_metal.mm`, after the aDiagTri `add_attr` at `:395-402`):
...
consumes `attrs[]`/`numFloats`, `:880-897`).
...
`uDynLightColor[4] (vec4 rgb_intensity)`, resolved at link (pattern: `:1413-1431`),
uploaded in `gfx_opengl_load_shader`/draw when dirty. Metal — extend the 48-byte
`Uniforms` (`gfx_metal.mm:289-297`) to a 288-byte struct (append: `float4x4 shadowMat`,
...
(`mtl_start_frame` `:1115` / GL equivalent before `ensure_scene_target` bind), the
...
- **Light matrix fit (single cascade — decision).** Ortho frustum centered on the
  ...
  Dam far plane ~200, `gfx_pc.c:15444` comment). Texel-snap the ortho origin to
...
- **Depth-only pass.** GL: `GL_DEPTH_COMPONENT24` texture FBO,
  ...
  `Depth32Float` descriptor pattern (`gfx_metal.mm:815-820`), own
...
- **Receiver injection (both generators).** Under `SHADER_OPT_SUN_SHADOW`, after the
  combiner clamp (GL: after `texel = clamp(...)`; MSL: after `gfx_metal.mm:539`) and
...
(`mtl_sampler_for` gains a comparison variant, `:982-1002`).
...
float ndl = max(dot(gN, -uSunDirWorld), 0.0);
float lit = uAmbientLuma + uSunLuma * ndl;                        // 0.588/0.412 as E1
texel.rgb *= mix(vec3(1.0), vec3(lit) / max(bakedLuma(vInputShade), 0.05), uRelightBlend);
```

Winding orientation: GE rooms are consistently wound for `G_CULL_BACK`
(`bg.c:6989-6990` sets it), so the derivative normal has stable sign per backend;
Metal's flipped y in `dFdy` is compensated by a per-backend constant (GL `+1`, Metal
`-1`) baked at generation time. `bakedLuma` needs the shade input; the generator knows
which `vInputN` is SHADE via `comb->shader_input_mapping` — pass its index into the
generator through a new `CCFeatures.shade_input_idx` filled in `gfx_cc_get_features`
(and `-1` when no shade input → skip luma-replace, plain multiply).
...
Hooked beside `gfx_pc.c:21061`: on a successful HD (or stock) settex decode, sidecars
upload to two additional `gfx_rapi->new_texture()` ids cached in the settex cache entry
(the cache already keys per token: `settex_cache` struct `gfx_pc.c:941-952`, per-token
lookup `:20692`). At bind time
...
`[[texture(n)]]`). **Verified**: units/indices 3+ are unused (GL `:1437-1449`; Metal
`:451-458` — snapshot uses 2, shadow map takes 5 per §4.4).
...
CPU-side at `gfx_pc.c:16245-16275` (per-vertex Lambert into `d->color`, lights
transformed via `calculate_normal_dir` / `rsp.current_lights_coeffs`, `:15257`).
...
### E1 — Smooth env normals (T1.3) — 13d
...
| ID | Task | Files | Steps | Acceptance | Est | Deps | Rails |
|---|---|---|---|---|---|---|---|
| W1.E6.T1 | Char normal capture + per-pixel path | `gfx_pc.c:16245+` (store `nrm` world-transformed), `:17288+` (CHRPROP bit `1u<<30`), both generators (normal-attr variant §4.4/§4.8) | CPU lighting kept as-is (feeds SHADE + identity); FS replaces at blend=1 | Guard close-up A/B (`--level dam`, engage a guard): banding on limbs/face smoothed; sun-shadow received on characters; viewmodel untouched (WEAPON class excluded — roadmap §8); identity-off byte-identical | 6 | E2.T3, E3.T4 | R3: `GE007_CHR_NORMALS` |
