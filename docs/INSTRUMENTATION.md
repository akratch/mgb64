...
| `GE007_CAMERA_SEED_WALK=1` | **Opt-in T13 experiment (default OFF).** For a detached authored camera (INTRO/FADESWIRL/SWIRL) also *walk* the BFS from the camera's resolved room, not just admit it — otherwise the walk stops at the camera room + its one-hop supplement neighbors and rooms further down the camera's sightline render as flat sky-blue (Silo intro headline defect). Setting `=1` fills them (Silo intro blue-clear pixels 3639→124, Dam 4050→46). **Default OFF because it is not sim-invariant:** more admitted rooms → more `PROPFLAG_ONSCREEN` actors → a different `pcRandom` stream → the deterministic intro camera pick shifts and `intro_oracle_dam_route` diverges from stock (native without the walk matches stock exactly). Blocked on the M2.3 render/sim decoupling. Complements `GE007_NO_CAMERA_SEED_FIX` (which disables the whole camera-room seed). |
