...
| `GE007_NO_CAMERA_SEED_WALK=1` | **A/B opt-out for the T13 fix (default-on).** For a detached authored camera (INTRO/FADESWIRL/SWIRL) the visibility BFS now *walks* from the camera's resolved room, not just admits it — otherwise the walk stops at the camera room + its one-hop supplement neighbors and rooms further down the camera's sightline render as flat sky-blue (Silo intro headline defect). Setting `=1` restores the pre-T13 admit-only behavior (negative control: Silo intro blue-clear pixels 124→3639). Complements `GE007_NO_CAMERA_SEED_FIX` (which disables the whole camera-room seed). |
