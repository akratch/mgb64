...
| D31 | **Landed 2026-07-08** (`510e181`); residual live | Intro Bond hovered ~57u above the floor during the swirl, then snapped down when phase 3 took over. Root motion is now unpinned across all three intro animation phases. Fresh ares retest on 2026-07-22 still measures 42 `cam_delta[1]` divergences (max 74.89u), so the scoped D31 waiver remains pending residual attribution. |
| D32 | Open; re-tested live 2026-07-22 | Fresh Dam ares/native pair still differs in swirl mode duration by 3 records. The D31/D43 landings did not obsolete this waiver. |
...
| D35 | Open; re-tested live 2026-07-22 | Camera-anchor drift remains after D43. Fresh Dam evidence measures divergences in 13 scoped fields (for example `cam_pos[0]` max 38.0043u and `cam_floor[1]` max 440.85u). The old claim that D35 was fully attributable to the missing D36 animation is refuted; residual root-cause work remains. |
| D36 | **Verified fixed 2026-07-22** | D43 restored the missing phase-3 `PlayAnimation`. A fresh current-HEAD vs stock-ares capture found zero divergences in the final scoped `gunhand` field, so its last waiver was removed. `intro_oracle_dam_bond_anim` now gates the full three-phase animation: scheduling-dependent onset is event-aligned, but post-onset field drift still fails. |
...
| D41 | Open; re-tested live 2026-07-22 | Fresh stock-ares/native Dam capture still measures 108 animation-frame divergences (max 1.0 frame; the dominant offset is 0.25 frame). The tick-offset entry point still needs tracing and alignment. |
