...
| D42 | Fixed (seed); walk root-caused, blocked opt-in | Establishing shots rendered near-blank (only the spawn room was admitted; detached-camera supplemental room seeding was skipped). Seed half fixed: seed the room BFS from the camera's own resolved room (`GE007_NO_CAMERA_SEED_FIX` hatch), verified blank→full geometry. **T13 (2026-07-09) root-caused the residual** (headline Silo defect: large sky-blue areas / blue behind Bond): the seed step admits the camera room but never *walks* its portals, so the BFS stops at the camera room + its one-hop supplement neighbors and everything one hop further down the sightline falls through to the clear color. Walking the BFS from the camera room fills it (opt-in `GE007_CAMERA_SEED_WALK=1`: Silo intro blue-clear pixels 3639→124, Dam 4050→46). **BLOCKED from default-on:** room admission is coupled to the sim — `room_rendered`→`getROOMID_isRendered`→`PROPFLAG_ONSCREEN` gates which actors tick and consume the deterministic RNG (`pcRandom`). Walking the camera room admits more rooms → more onscreen actors (Silo intro 2→5) → a non-stock intro RNG stream that shifts the deterministic camera-index pick (`intro_oracle_dam_route` diverges from ares/stock with it ON; native without the walk matches stock exactly → stock does not admit these rooms during the intro). Default-on needs the M2.3 render/sim decoupling first, or a combat-field oracle proving the fuller intro set is faithful. Diagnostic left in tree: `GE007_TRACE_ROOM_CLASSIFY` (dropped-vs-far). |
