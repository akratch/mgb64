...
| D42 | Fixed, residual open | Establishing shots rendered near-blank (only the spawn room was admitted; detached-camera supplemental room seeding was skipped). Fixed by seeding the room BFS from the camera's own resolved room (`GE007_NO_CAMERA_SEED_FIX` hatch); verified blank→full geometry. Residual: distant reservoir mountains not portal-reachable from the seeded room are still absent in far establishing shots — needs a stock pixel comparison and is assigned to M1.5. |
