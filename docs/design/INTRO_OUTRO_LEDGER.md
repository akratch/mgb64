...
| D42 | Fixed (seed + walk); narrow residual | Establishing shots rendered near-blank (only the spawn room was admitted; detached-camera supplemental room seeding was skipped). Fixed in two parts: (1) seed the room BFS from the camera's own resolved room (`GE007_NO_CAMERA_SEED_FIX` hatch), verified blank→full geometry; (2) **T13 (2026-07-09):** the seed step admitted the camera room but never *walked* its portals, so the BFS stopped at the camera room + its one-hop supplement neighbors and everything one hop further down the sightline fell through to the sky/clear color (the headline Silo defect: blue behind Bond). Fix walks the BFS from the camera room too (`GE007_NO_CAMERA_SEED_WALK` hatch). Silo intro blue-clear pixels 3639→124; Dam intro 4050→46 (the "distant reservoir/near-cliff" residual now largely fills). Remaining residual: rooms that are directly visible across open space but many winding-portal-hops away (Silo rooms 28/44 past a supplement-admitted room; a screen-edge sliver) are still dropped by the aperture walk — the M3.4 degenerate-aperture class, not the seed/walk class. |
