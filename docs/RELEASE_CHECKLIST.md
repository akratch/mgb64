...
- Run `scripts/check_github_launch_ready.sh` after GitHub settings are final.
  ...
  the current public branch, and scans public repository metadata, labels, issue
  text, PR comments, and Discussion text for high-risk private paths, stale
  handoff language, token-shaped strings, proprietary notice fragments, and
  resolvable stale commit references; treat findings there like
  repository-history contamination. GitHub keeps closed PR refs read-only; if any
  stale `refs/pull/*`
