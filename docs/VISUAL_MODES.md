...
Two lighting features are merged but **default-off** and **not** part of
`--remaster` yet — enable them explicitly to try them:
...
| *(env-only)* | `GE007_SHOOT_OUT_LIGHTS=1` | Shooting a ceiling light fixture darkens its surface (with a spark) and it stays dark — a faithfulness restoration of the original behavior. |

Both are gated so the all-off render is byte-identical, and the blend strength /
