...
*Addendum: the branch head is now `daf711e`, which adds only this handoff document itself —
the validation scope remains `9d6714d..c133b89`.*
...
| **V3** | **Faithful-base GL↔Metal parity** ~2.4–4.0%/level `[LONG, ~10–20 min: 1 pin run + 10 captures, incl. G1 GL retries]` | Pin config: `build/ge007 baserom.u.z64 --level dam --deterministic --screenshot-frame 2 --screenshot-exit --config-override Video.RemasterFX=0 --config-override Video.RenderScale=1 --config-override Video.MSAA=0 --config-override Video.Ssao=0 --background`. Then per level L ∈ {dam, facility, surface1, jungle, egyptian}: capture GL (`--screenshot-label g_L`) and Metal (`GE007_RENDERER=metal`, `--screenshot-label m_L`) at `--screenshot-frame 30`, then `python3 tools/compare_screenshots.py screenshot_g_L.bmp screenshot_m_L.bmp --exclude-region 470,72,175,110` | dam ≈4.0%, facility ≈2.8%, surface1 ≈2.8%, jungle ≈2.8%, egyptian ≈2.7% (±0.3pp). The exclude-region masks the GL-only minimap |
...
| **V9** | ASan clean on Metal + SSAO + RDP paths `[LONG, ~5–10 min incl. the ASan rebuild]` | `cmake --build build-asan --parallel 8`, then `ASAN_OPTIONS=detect_leaks=0 GE007_RENDERER=metal timeout 180 build-asan/ge007 baserom.u.z64 --remaster --level surface1 --deterministic --screenshot-frame 40 --screenshot-exit --background 2>&1 \| grep -iE 'AddressSanitizer\|SUMMARY'` | No output (no findings); run completes |
...
| **V14** | Campaign-wide no-hang `[LONG, ~20 min]` | Loop the 18 solo levels (`dam facility runway surface1 bunker1 silo frigate statue archives streets depot train jungle control caverns cradle aztec egyptian` — slugs per `main_pc.c` `kPcStartStages`; `surface`/`bunker` alone are NOT valid) with `GE007_RENDERER=metal timeout 120 … --remaster --level $lvl --deterministic --screenshot-frame 25 --screenshot-exit --background`, count `Auto-screenshot complete` | 18/18 |
...
| Metal-default-on-macOS flip decision | W8 doc §4.1 |
| Branch merge-train (5 branches → `main`) — **program task #1** | W8.E6.T2 (doc §4.7) |
